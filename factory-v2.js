...
function checks(p){
  return [
    ...
    ["Precio / consultar",p.products.some(x=>x.name),"Cada oferta mostrará precio o Consultar."]
  ];
}
function filtered(){
  let a=[...S.projects];
...
function stepBusiness(p){
  ...
  <div class="profileBox"><b>PERFIL DE PRODUCCIÓN</b><small>Elegí un rubro y la fábrica prepara estructura, tono y ofertas.</small><div class="profileGrid"><button class="${p.profile==="Panadería"?"selected":""}" onclick="applyProfile('Panadería')">Panadería</button><button class="${p.profile==="Gastronomía"?"selected":""}" onclick="applyProfile('Gastronomía')">Gastronomía</button><button class="${p.profile==="Peluquería"?"selected":""}" onclick="applyProfile('Peluquería')">Peluquería</button><button class="${p.profile==="Fotografía"?"selected":""}" onclick="applyProfile('Fotografía')">Fotografía</button><button class="${p.profile==="Artesanía"?"selected":""}" onclick="applyProfile('Artesanía')">Artesanía</button><button class="${p.profile==="Productor"?"selected":""}" onclick="applyProfile('Productor')">Productor</button><button class="${p.profile==="Profesor"?"selected":""}" onclick="applyProfile('Profesor')">Profesor</button><button class="${p.profile==="Servicios"?"selected":""}" onclick="applyProfile('Servicios')">Servicios</button><button class="${p.profile==="Tienda"?"selected":""}" onclick="applyProfile('Tienda')">Tienda</button></div><label class="field compact"><b>Modelo comercial</b><select onchange="applyProfile('${p.profile||""}',this.value)"><option value="">Base del rubro</option><option>Mostrador local</option><option>Pedidos y tortas</option><option>Producción artesanal</option><option>Menú diario</option><option>Comida para llevar</option><option>Eventos y reservas</option><option>Turnos rápidos</option><option>Color y tratamientos</option><option>Experiencia premium</option><option>Sesiones</option><option>Producto y comercio</option><option>Eventos</option><option>Regalos</option><option>Personalizados</option><option>Colección</option><option>Venta directa</option><option>Cajas y combos</option><option>Temporada</option><option>Clases</option><option>Apoyo</option><option>Talleres</option><option>Servicio directo</option><option>Presupuesto</option><option>Servicio premium</option><option>Catálogo</option><option>Promociones</option><option>Productos destacados</option></select></label></div>
...
function exportSite(){
  const p=active();personalize(p);const t=THEMES[p.style]||THEMES.calido,offers=p.products.filter(x=>x.name),link=wa(p.whatsapp),inst=ig(p.instagram),esc2=esc;
  const heroClass=p.heroStyle==="minimal"?"minimal":p.heroStyle==="immersive"?"immersive":"photo";
  const layout=p.layout==="featured"?"featured":p.layout==="list"?"list":"cards";
  const html=`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="${esc2(p.description||p.name)}"><meta name="theme-color" content="${t.ink}"><title>${esc2(p.name||"Mini-web")}</title><style>
:root{--ink:${t.ink};--accent:${t.accent};--paper:${t.paper}}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:#202522;font-family:system-ui,-apple-system,Segoe UI,sans-serif}.hero{min-height:58vh;background:#ddd center/cover;display:flex;align-items:end;position:relative;color:#fff}.hero.minimal{background:var(--paper);color:var(--ink)}.hero.immersive{min-height:72vh}.shade{position:absolute;inset:0;background:linear-gradient(transparent,rgba(0,0,0,.78))}.minimal .shade{display:none}.copy{position:relative;z-index:1;max-width:980px;width:100%;margin:auto;padding:38px 24px 34px}.copy small{font-size:10px;letter-spacing:.16em;font-weight:900;text-transform:uppercase}.copy h1{font-size:clamp(38px,7vw,72px);line-height:.95;margin:8px 0;letter-spacing:-.055em}.copy p{max-width:720px;font-size:16px;line-height:1.55;margin:0}.body{max-width:980px;margin:auto;padding:28px 22px 50px}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:32px}.actions a,.card a{background:var(--ink);color:#fff;text-decoration:none;padding:11px 15px;border-radius:999px;font-weight:850;font-size:12px}.actions a.light{background:#fff;color:var(--ink)}.sectionTitle h2{font-size:30px;margin:5px 0 16px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.grid.featured{grid-template-columns:1fr}.grid.list{grid-template-columns:1fr}.card{background:#fff;border:1px solid #e7e1d8;border-radius:18px;overflow:hidden}.grid.featured .card:first-child{display:grid;grid-template-columns:1.1fr 1fr}.grid.list .card{display:grid;grid-template-columns:1fr 1.7fr}.card img{width:100%;height:210px;object-fit:cover}.grid.featured .card:first-child img,.grid.list .card img{height:100%;min-height:170px}.card div{padding:17px}.card h3{font-size:18px;margin:0 0 5px}.card p{color:#6f716e;line-height:1.5;font-size:13px}.price{display:block;font-size:16px;margin:12px 0}.info{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:28px}.info div{background:rgba(255,255,255,.72);padding:15px;border-radius:15px}.info p{margin:7px 0 0;font-size:13px;white-space:pre-line}footer{text-align:center;color:#92928d;font-size:10px;padding:22px}@media(max-width:650px){.grid,.info{grid-template-columns:1fr}.grid.featured .card:first-child,.grid.list .card{display:block}.hero{min-height:62vh}.copy{padding:28px 18px}.body{padding:22px 16px 40px}}
</style></head><body><section class="hero ${heroClass}" ${heroClass==="minimal"?"":"style=\"background-image:url('"+p.hero+"')\""}><div class="shade"></div><div class="copy"><small>${esc2(p.category||"Negocio local")}</small><h1>${esc2(p.name||"Tu negocio")}</h1><p>${esc2(p.tagline||p.description||"")}</p></div></section><main class="body"><div class="actions">${link?`<a href="${link}" target="_blank" rel="noopener">WHATSAPP</a>`:""}${p.showInstagram&&inst?`<a class="light" href="${inst}" target="_blank" rel="noopener">INSTAGRAM</a>`:""}</div><div class="sectionTitle"><small>Oferta</small><h2>Lo que ofrecemos</h2></div><div class="grid ${layout}">${offers.length?offers.map(x=>`<article class="card">${x.image?`<img src="${x.image}" alt="${esc2(x.name)}">`:""}<div><h3>${esc2(x.name)}</h3><p>${esc2(x.description)}</p>${p.showPrice?`<span class="price"><b>${esc2(x.price||"Consultar")}</b></span>`:""}${link?`<a href="${link}?text=${encodeURIComponent("Hola, vi tu página y quiero consultar por "+x.name)}" target="_blank" rel="noopener">${esc2(p.cta||"Consultar por WhatsApp")}</a>`:""}</div></article>`).join(""):'<div>Próximamente.</div>'}</div><div class="info">${p.showAddress&&p.address?`<div><small>${esc2(p.locationLabel||"Ubicación")}</small><p>${esc2(p.address)}</p></div>`:""}${p.showHours&&p.hours?`<div><small>Horarios</small><p>${esc2(p.hours)}</p></div>`:""}</div></main><footer>${esc2(p.footerNote||"Mini-web producida con Fábrica Ocarina")}</footer></body></html>`;
